...
export function Editcadastrolocatario() {
    ...
    async function fetchUserData() {
      try {
        const response = await axios.get(`${import.meta.env.VITE_API_URL}/locador/profile`, {
          headers: { Authorization: `Bearer ${auth.tokens?.authToken.token}` },
        });
  ...
  const handleUpdateProfile = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await signup(nome, email, password, identificador, rua, cep, estado, cidade, telefone);
      ...
      {!successMessage && (
        <div className="flex justify-center items-center h-screen">
          <div className="p-6 bg-white shadow-md rounded-lg mb-3 w-full sm:w-3/4 md:w-1/2 lg:w-1/3 xl:w-1/4">
            ...
            <form onSubmit={handleUpdateProfile} className="flex flex-wrap">
              ...
              <div className="w-full px-2 mt-3">
                <div className="flex justify-between">
                  <button
                    ...
                    onClick={() => navigate('/locatarioPage')}
                  >
                    Voltar
                  </button>
                  <button
...
export default Editcadastrolocatario;
